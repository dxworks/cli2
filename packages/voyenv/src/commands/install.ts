...
import { Voyenv } from '../model/voyenv.js';
import { VoyagerService } from '../services/voyager.service.js';
import { InstrumentService } from '../services/instrument.service.js';
import { Octokit } from 'octokit';
import path from 'node:path';
import { defaultOctokit, log } from '@dxworks/common';
...
export const voyenvInstall = new Command()
  .name('install')
  .description(
    'Will download and install a voyager release as well as its instruments and runtimes as specified in the configuration file',
  )
  .argument(
    '[voyenvFilePath]',
    'The voyenv file with the configuration of the voyager release. By default, it is voyenv.yml',
    defaultVoyenvFileName,
  )
  .action(install);
...
export async function install(file: string, _options: unknown): Promise<void> {
  ...
  await new VoyagerService(octokit, rootDir).downloadAndInstallVoyager(
    voyenv.voyager_version,
  );
  log.info('Downloading Instruments');
  await new InstrumentService(octokit, rootDir).downloadInstruments(
    voyenv.instruments,
  );
  ...
  if (voyenv.name !== '.') {
    ...
    voyenv.name = '.';
    fs.writeFileSync(
      path.resolve(rootDir, defaultVoyenvFileName),
      YAML.stringify(voyenv),
      { encoding: 'utf-8' },
    );
  }
